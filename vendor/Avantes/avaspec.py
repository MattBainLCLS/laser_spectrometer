...
def _load_avantes_lib():
    import ctypes as ctypes  # explicit local binding avoids UnboundLocalError
    _here = os.path.dirname(os.path.abspath(__file__))

    if 'linux' in sys.platform:
        candidates = [
            os.path.join(_here, "libavs.so.0"),
            "/usr/local/lib/libavs.so.0",
        ]
        _func = ctypes.CFUNCTYPE
    elif 'darwin' in sys.platform:
        candidates = [
            os.path.join(_here, "libavs.0.dylib"),
        ...
        if ctypes.sizeof(ctypes.c_voidp) == 8:
            candidates = [os.path.join(_here, "avaspecx64.dll"), "./avaspecx64.dll"]
            _WM = 0x8001
        else:
            candidates = [os.path.join(_here, "avaspec.dll"), "./avaspec.dll"]
            _WM = 0x0401
        globals()["WM_MEAS_READY"] = _WM
        ...
        if os.path.exists(path):
            try:
                if sys.platform.startswith('win'):
                    return ctypes.WinDLL(path), _func
                else:
                    return ctypes.CDLL(path), _func
            except OSError:
                continue

    # Nothing found — raise a helpful error
    if 'darwin' in sys.platform:
        lib_name = "libavs.0.dylib  (+ libavs.9.x.x.x.dylib)"
    elif 'linux' in sys.platform:
        lib_name = "libavs.so.0"
    else:
        lib_name = "avaspecx64.dll / avaspec.dll"

    raise OSError(
        f"Avantes native library not found ({lib_name}).\n"
        f"Copy the library file(s) from your Avantes SDK installation into the "
        f"Avantes/ folder of this project, or install them to /usr/local/lib/.\n"
        f"The SDK can be downloaded from https://www.avantes.com/support/software/"
    )

lib, func = _load_avantes_lib()
