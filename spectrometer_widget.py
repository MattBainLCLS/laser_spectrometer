"""
Reusable spectrometer widget.

Handles device discovery, live spectrum display, grab/free-run, time-domain
view, reference pinning, save and interval acquisition. Can be embedded in
a larger window or run standalone via SpectrometerWindow in spectrometer_gui.py.

Public API
----------
spec              — connected SpectrometerBase instance, or None
status_message    — signal(str): status-bar text for the host window
shutdown()        — stop workers and close the device
"""
...
from PyQt6.QtWidgets import (
    ...
    QMessageBox, QFileDialog, QCheckBox, QComboBox, QLineEdit, QGroupBox,
)
...
def compute_time_domain(wavelengths_nm, spectrum, smooth_sigma=0):
    ...
    freq_s, intens_s = freq[order], np.maximum(np.asarray(spectrum)[order], 0.0)
...
class SpectrometerWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        # Time domain panel (hidden until toggled)
        ...
        self.td_window_spin.setRange(1, 100_000)
        ...
        self.td_panel.setFixedWidth(self._TD_PANEL_WIDTH)
    ...
    def _connect_spectrometer(self):
        ...
        if self.spec is None:
            QMessageBox.critical(self, "No device",
                                 "No supported spectrometer found.")
            return
        ...
        self.status_message.emit(
            ...
            f"Exposure range: {self.spec.min_exposure_time:.4f}–"
            f"{self.spec.max_exposure_time:.1f} s")
    ...
    def toggle_time_domain(self, checked):
        ...
        window = self.window()
        window.resize(window.width() + (delta if checked else -delta),
                      window.height())
        if self.worker and self.worker.isRunning():
            self.worker.td_params = (
                (self.wavelengths, self.td_smooth_spin.value()) if checked else None)
    ...
    def _on_td_ready(self, t_fs, I_t, dt_fs):
        ...
        self.td_dt_label.setText(
            f"dt: {dt_fs * 1000:.0f} as" if dt_fs < 1 else f"dt: {dt_fs:.2f} fs")

    def _update_time_domain(self, data):
        t_fs, I_t, dt_fs = compute_time_domain(
            self.wavelengths, data.spectrum,
            smooth_sigma=self.td_smooth_spin.value())
    ...
    def _on_interval_spectrum(self, data, index):
        ...
        stem = os.path.join(output_dir,
                            f"{self.spec.serial_number}_{ts}_{index:04d}")
        fmt = self.format_combo.currentText()
        if fmt in ("CSV", "Both"):
    ...
    def pin_reference(self):
        ...
        self.status_message.emit("Reference pinned.")
    ...
    def save_png(self):
        ...
        self.status_message.emit("Saved spectrum.png")
    ...
    def save_csv(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save CSV", "spectrum.csv", "CSV files (*.csv)")
        if not path:
            return
